            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                ...
                <div class="mt-4 md:mt-0 flex space-x-2">
                    <div class="space-y-2 sm:space-y-0 sm:flex sm:items-center sm:justify-end sm:gap-2">
                        ...
                        @if (!$colocation->is_owner && $colocation->is_active)
                            <form action="{{ route('colocations.members.leaving-auth', $colocation) }}" method="POST"
                                onsubmit="return confirm('{{ __('Êtes-vous sûr de vouloir quitter cette colocation ?') }}')"
                                class="block sm:inline-block w-full sm:w-auto">
                                @csrf
                                @method('PATCH')
                                <button type="submit"
                        ...
                        <a href="{{ route('expenses.export') }}"
                            ...
                            <svg class="w-5 h-5 sm:w-4 sm:h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
