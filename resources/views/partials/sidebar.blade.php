    ...
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white flex flex-col justify-between">
        <div>
            <ul class="space-y-2 font-medium">
                <li>
                    <a href="{{ route('dashboard') }}" class="flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 group {{ request()->is('dashboard*') ? 'bg-gray-100' : '' }}">
        ...
        <div class="pt-4 mt-4 border-t border-gray-200">
            <div class="p-4 rounded-xl bg-gray-50 border border-gray-100 shadow-sm">
                @php
                    ...
                    $bgClass = $isPositive ? 'bg-green-100 text-green-800' : ($isNegative ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-600');
                    $prefix = $isPositive ? '+' : '';
                @endphp
